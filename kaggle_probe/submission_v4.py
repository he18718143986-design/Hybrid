...
TOTAL_STEPS = 500
SIM_HORIZON = 110
ROUTE_SEARCH_HORIZON = 80           # early/mid intercept search
ROUTE_SEARCH_HORIZON_LATE = 55      # late-game: shorter search, ~30% faster intercept
HORIZON = SIM_HORIZON
LAUNCH_CLEARANCE = 0.1

# Late-game / congested-board performance budget (Kaggle actTimeout=1.0)
LATE_GAME_STEP_THRESHOLD = 280      # step >= this → tighten planner
CONGESTED_FLEET_COUNT = 10          # many in-flight fleets → tighten
CONGESTED_PLANET_COUNT = 26
...
MULTI_SOURCE_TOP_K = 4              # was 6 — smaller combo search
MULTI_SOURCE_TOP_K_LATE = 3
MULTI_SOURCE_ETA_TOLERANCE = 2
MULTI_SOURCE_PLAN_PENALTY = 0.97
HOSTILE_SWARM_ETA_TOLERANCE = 2
THREE_SOURCE_SWARM_ENABLED = True
THREE_SOURCE_SWARM_LATE_ENABLED = False  # 3-source O(n³) off when board is busy
THREE_SOURCE_MIN_TARGET_SHIPS = 20
THREE_SOURCE_ETA_TOLERANCE = 1
THREE_SOURCE_PLAN_PENALTY = 0.93
FOUR_SOURCE_SWARM_ENABLED = False   # O(n⁴) — disabled on Kaggle 1s budget
...
# NOTE: tuned 2026-05-24 — original 0.5/CAP=none was overestimating support and
# rejecting too many feasible captures (need > src_available). Now uses v12-style
# light projection plus a margin in preferred_send for double-safety.
HOSTILE_REINFORCE_FRACTION = 0.25   # was 0.5 — committed fraction of allied garrison
HOSTILE_REINFORCE_MAX_SOURCES = 2   # was 3 — cap modeled reinforcing allies per target
HOSTILE_REINFORCE_MIN_SHIPS = 6     # was 4 — ignore tinier allies (less noise)
HOSTILE_REINFORCE_SLACK = 0         # extra turns beyond our ETA we still model
HOSTILE_REINFORCE_TOTAL_CAP = 18    # NEW — hard cap on total assumed reinforcement ships

# v12-style margin (additional safety in preferred_send, does NOT inflate need)
HOSTILE_REINFORCE_MARGIN_HORIZON = 8   # turns past arrival to consider for margin
HOSTILE_REINFORCE_MARGIN_RATIO = 0.20  # fraction of nearby allied ships
HOSTILE_REINFORCE_MARGIN_CAP = 12      # cap on margin ships
HOSTILE_REINFORCE_LATE_ENABLED = False  # skip expensive B1 timeline model when busy
...
SOFT_ACT_BUDGET_RATIO = 0.78       # was 0.82 — extra headroom for Kaggle's slower servers
SOFT_ACT_BUDGET_MIN = 0.55         # safety floor when actTimeout is unusually small
HEAVY_PHASE_MIN_TIME = 0.16
HEAVY_PHASE_MIN_TIME_LATE = 0.22    # need more slack before starting heavy work late game
OPTIONAL_PHASE_MIN_TIME = 0.08
OPTIONAL_PHASE_MIN_TIME_LATE = 0.12
HEAVY_ROUTE_PLANET_LIMIT = 24      # was 32
HEAVY_ROUTE_PLANET_LIMIT_LATE = 18

# Set each turn by plan_moves; read by intercept search helpers.
_perf_tight = False


def _set_perf_context(world):
    global _perf_tight
    _perf_tight = (
        world.step >= LATE_GAME_STEP_THRESHOLD
        or world.is_late
        or len(world.fleets) >= CONGESTED_FLEET_COUNT
        or len(world.planets) >= CONGESTED_PLANET_COUNT
    )
...
def _route_search_horizon():
    return ROUTE_SEARCH_HORIZON_LATE if _perf_tight else ROUTE_SEARCH_HORIZON


def _use_hostile_reinforce():
    if not HOSTILE_REINFORCE_LATE_ENABLED and _perf_tight:
        return False
    return True


def _capture_targets(world):
    """Reduce O(sources × targets) work when the board is busy."""
    if not _is_perf_tight():
        return world.planets
    targets = list(world.enemy_planets)
    seen = {p.id for p in targets}
    for planet in world.neutral_planets:
        if planet.id in seen:
            continue
        if planet.id in world.comet_ids:
            if world.comet_life(planet.id) >= 8:
                targets.append(planet)
                seen.add(planet.id)
            continue
        if planet.production >= 2 or is_static_planet(planet):
            targets.append(planet)
            seen.add(planet.id)
    return targets
...
def search_safe_intercept(src, target, ships, initial_by_id, ang_vel, comets, comet_ids):
    best = None
    best_score = None
    max_turns = min(HORIZON, _route_search_horizon())
    if target.id in comet_ids:
...
def fleet_target_planet(fleet, planets, initial_by_id, angular_velocity, comets, comet_ids):
    """
    Infer which planet a fleet is heading toward.

    Static planets: use analytical ray-vs-circle intersection (fast, exact).
    Rotating planets / comets: step through integer turns and check whether
    the fleet's projected position falls inside the planet's predicted disc.
    Returns (planet, integer_eta) or (None, None).
    """
...
def build_arrival_ledger(fleets, planets, initial_by_id, angular_velocity, comets, comet_ids):
    arrivals_by_planet = {planet.id: [] for planet in planets}
    for fleet in fleets:
        target, eta = fleet_target_planet(
            fleet, planets, initial_by_id, angular_velocity, comets, comet_ids,
        )
        if target is None:
            continue
...
class WorldModel:
    def __init__(self, player, step, planets, fleets, initial_by_id, ang_vel, comets, comet_ids):
        ...
        self.arrivals_by_planet = build_arrival_ledger(
            fleets, planets, initial_by_id, ang_vel, comets, self.comet_ids,
        )
        self.base_timeline = {
    ...
    def hostile_reinforcement_arrivals(self, target_id, attack_turn):
        """
        Estimate plausible reinforcement arrivals from the target owner's other
        planets that could land at or before our attack. Used to make `need`
        calculations account for enemy self-defense (B1).

        Returns a tuple of (turns, owner, ships) extra-arrival events. Empty
        tuple for neutrals or for our own planets.
        """
        target = self.planet_by_id[target_id]
        owner = target.owner
        ...
        attack_turn = max(1, int(math.ceil(attack_turn)))
        cache_key = (target_id, attack_turn)
        cached = self.hostile_reinforce_cache.get(cache_key)
        if cached is not None:
            return cached

        candidates = []
        cutoff = attack_turn + HOSTILE_REINFORCE_SLACK
        for ally in self.planets:
            if ally.id == target_id or ally.owner != owner:
                continue
            send_cap = int(int(ally.ships) * HOSTILE_REINFORCE_FRACTION)
            if send_cap < HOSTILE_REINFORCE_MIN_SHIPS:
                continue
            seeded = self.best_probe_aim(ally.id, target_id, send_cap)
            if seeded is None:
                continue
            send_used, aim = seeded
            eta = aim[1]
            if eta > cutoff:
                continue
            candidates.append((eta, int(send_used)))

        candidates.sort()
        candidates = candidates[:HOSTILE_REINFORCE_MAX_SOURCES]
        # Hard cap total ships across all modeled reinforcers — prevents pathological
        # large `need` from inflating attack rejection rate.
        capped = []
        ships_budget = HOSTILE_REINFORCE_TOTAL_CAP
        for eta, ships in candidates:
            if ships_budget <= 0:
                break
            allowed = min(ships, ships_budget)
            if allowed >= HOSTILE_REINFORCE_MIN_SHIPS:
                capped.append((eta, allowed))
                ships_budget -= allowed
        result = tuple((eta, owner, ships) for eta, ships in capped)
        self.hostile_reinforce_cache[cache_key] = result
        return result
    ...
    def min_ships_to_own_by(
        ...
        if model_hostile_reinforce and attacker_owner == self.player:
            target = self.planet_by_id[target_id]
            if target.owner not in (-1, self.player):
                hostile_extras = self.hostile_reinforcement_arrivals(target_id, eval_turn)
                if hostile_extras:
                    extra_arrivals = tuple(extra_arrivals) + hostile_extras
...
def detect_enemy_planet_battles(world):
    """
    Detect enemy-vs-enemy battles on enemy planets — these create attack windows
    because the winner is weakened. Uses the full forward-simulation combat
    resolver (resolve_arrival_event) for accurate multi-side post-battle state.
    """
    battles = []
    seen = set()
    for target in world.enemy_planets:
        attacker_etas = sorted({
            int(math.ceil(eta))
        ...
        if not attacker_etas:
            continue

        for eta in attacker_etas:
            key = (target.id, eta)
            if key in seen:
                continue
            seen.add(key)
            owner_after, ships_after = world.projected_state(target.id, eta)
            if ships_after < 25:
                battles.append({
                    "target_id": target.id,
                    "battle_turn": eta,
                    "post_owner": owner_after,
                    "post_ships": int(math.floor(ships_after)),
...
def build_policy_state(world, deadline=None):
    ...
    for target in world.planets:
        ...
        if _is_perf_tight():
            if target.owner == -1:
                if target.production < 2 and not is_static_planet(target):
                    continue
                if target.id in world.comet_ids and world.comet_life(target.id) < 8:
                    continue
    ...
    for planet in world.my_planets:
        ...
        # Endgame: in the very-late stage, garrison stops mattering — every idle
        # ship that could be capturing is wasted. Release everything except what's
        # strictly required to survive the next few combat resolutions.
...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    ...
    if world.is_four_player and world._weakest_enemy and target.owner == world._weakest_enemy:
        margin = max(0, margin - 2)
    # v12-style hostile reinforcement margin — additive only (does NOT inflate need
    # in min_ships_to_own*; this complements the heavier timeline-based model with
    # a small safety pad so we don't get caught by surprise reinforcements).
    if (
        ...
        for ally in world.enemy_planets:
            if ally.owner != target_owner or ally.id == target.id:
                continue
            ally_ships = max(1, int(ally.ships))
            est = estimate_arrival(
                ally.x, ally.y, ally.radius,
            ...
            if est is None:
                continue
            ally_eta = est[1]
            if ally_eta <= arrival_turns + HOSTILE_REINFORCE_MARGIN_HORIZON:
                reinforce_est += int(ally_ships * HOSTILE_REINFORCE_MARGIN_RATIO)
        margin += min(HOSTILE_REINFORCE_MARGIN_CAP, reinforce_est)
...
def settle_plan(
    ...
    def evaluate(send):
        ...
        need = world.min_ships_to_own_by(
            ...
            planned_commitments=planned_commitments,
            upper_bound=src_cap,
            model_hostile_reinforce=_use_hostile_reinforce(),
        )
...
def plan_moves(world, deadline=None):
    ...
    def allow_heavy_phase():
        planet_limit = (
            HEAVY_ROUTE_PLANET_LIMIT_LATE if _is_perf_tight() else HEAVY_ROUTE_PLANET_LIMIT
        )
        min_time = HEAVY_PHASE_MIN_TIME_LATE if _is_perf_tight() else HEAVY_PHASE_MIN_TIME
        return time_left() > min_time and len(world.planets) <= planet_limit

    def allow_optional_phase():
        min_time = OPTIONAL_PHASE_MIN_TIME_LATE if _is_perf_tight() else OPTIONAL_PHASE_MIN_TIME
        return time_left() > min_time

    def allow_swarm_phase():
        if not allow_heavy_phase():
            return False
        if _is_perf_tight() and not THREE_SOURCE_SWARM_LATE_ENABLED:
            return False
        return True

    def multi_source_top_k():
        return MULTI_SOURCE_TOP_K_LATE if _is_perf_tight() else MULTI_SOURCE_TOP_K

    attack_targets = _capture_targets(world)
    ...
    for src in world.my_planets:
        ...
        for target in attack_targets:
            ...
            if partial_send_cap >= PARTIAL_SOURCE_MIN_SHIPS and allow_swarm_phase():
    ...
    if allow_swarm_phase():
      for target_id, options in source_options_by_target.items():
        ...
        # 4-source coordinated swarm — only for very heavy targets where 3-source
        # would not suffice; the goal is to overwhelm fortified enemy planets in
        # late game / endgame pushes.
        if (
            ...
            for i in range(len(top_options)):
                for j in range(i + 1, len(top_options)):
                    for k in range(j + 1, len(top_options)):
                        for m in range(k + 1, len(top_options)):
                            if expired():
                                return finalize_moves()
                            quad = [
                                top_options[i],
                            ...
                            if len({option.src_id for option in quad}) < 4:
                                continue
                            quad_tol = max(
                                FOUR_SOURCE_ETA_TOLERANCE,
                                swarm_eta_tolerance(tuple(quad), target, world),
                            )
                            turns_list = [option.turns for option in quad]
                            if max(turns_list) - min(turns_list) > quad_tol:
                                continue
                            ...
                            if need <= 0 or total_cap < need:
                                continue
                            # Skip if any subset of 3 would already work — those
                            # missions are already enqueued and cheaper to execute.
                            if any(
                                quad[a].send_cap + quad[b].send_cap + quad[c].send_cap >= need
                                for a in range(4)
    ...
    if not world.is_very_late and not _is_perf_tight() and allow_optional_phase():
        for src in world.my_planets:
            if expired():
                return finalize_moves()
    ...
    # === TOTAL WAR ENDGAME ===
    # Value/cost ranked (src, target) candidates instead of distance-only greedy.
    # Sizes attacks via min_ships_to_own_at + small margin, registers commitments
    # so multiple my_planets don't pile on the same target.

    if world.is_total_war and world.enemy_planets and allow_optional_phase():
        if world._weakest_enemy is not None:
            weakest_planets = [
                p for p in world.enemy_planets if p.owner == world._weakest_enemy
            ]
            primary_targets = weakest_planets or world.enemy_planets
        else:
            primary_targets = world.enemy_planets

        target_sets = [primary_targets]
        if primary_targets is not world.enemy_planets:
            target_sets.append(world.enemy_planets)
        ...
        for target_set in target_sets:
            ...
            for src in world.my_planets:
                ...
                for target in target_set:
                    if target.owner == world.player:
                        continue
                    aim = world.plan_shot(src.id, target.id, atk_left)
                    if aim is None:
                        continue
                    angle, turns, _, _ = aim
                    ...
                    need = world.min_ships_to_own_at(
                        ...
                        planned_commitments=planned_commitments,
                        upper_bound=atk_left,
                        model_hostile_reinforce=_use_hostile_reinforce(),
                    )
                    if need <= 0 or need > atk_left:
                        continue
                    saved_turns = max(1, world.remaining_steps - turns)
                    value = target.production * saved_turns + max(0, int(target.ships)) * 0.4
                    if target.id in world.exposed_planet_ids:
                        value *= 1.4
                    score = value / (need + turns * 0.4 + 1.0)
                    candidates.append((score, src.id, target.id, angle, turns, need, atk_left))
            ...
            for score, src_id, target_id, angle, turns, need, _atk_left_at_eval in candidates:
                if expired():
                    return finalize_moves()
                if src_id in used_src:
                    continue
                left = source_attack_left(src_id)
                if left < need:
                    continue
                send = min(left, need + FINISHING_HOSTILE_SEND_BONUS)
                actual = append_move(src_id, angle, send)
                if actual < need:
                    continue
                planned_commitments[target_id].append((turns, world.player, int(actual)))
    ...
    # === ENDGAME IDLE-SHIP DISPATCHER ===
    # Final ~25 turns: any remaining idle ship that can reach a non-friendly
    # planet in time should capture it. This catches ships left behind by the
    # main mission scheduler (e.g. when need > src_available and we couldn't
    # afford a capture earlier, now there's no benefit to keeping them home).
    if world.is_very_late and allow_optional_phase():
        non_friendly = [p for p in world.planets if p.owner != world.player]
        if non_friendly:
            for src in world.my_planets:
                if expired():
                    return finalize_moves()
                ...
                # Score targets: prefer weakest enemy planets > weak neutrals.
                best = None  # (priority, score, target, angle, turns)
                for target in non_friendly:
                    ...
                    if turns >= world.remaining_steps - 1:
                        continue
                    # Will atk_left be enough?
                    owner_after, _ = world.projected_state(
                        target.id,
                        turns,
                        extra_arrivals=((turns, world.player, atk_left),),
                    )
                    if owner_after != world.player:
                        continue
                    # Priority: enemy weakest > enemy other > neutral
                    if target.owner == world._weakest_enemy and target.owner != -1:
                        pri = 0
                    elif target.owner not in (-1, world.player):
                        pri = 1
                    else:
                        pri = 2
                    profit = target.production * max(1, world.remaining_steps - turns)
                    score = profit / max(1.0, turns + 1.0)
                    key = (pri, -score)
                    if best is None or key < best[0]:
                        best = (key, target, angle, turns)

                if best is None:
                    continue
                _, target, angle, turns = best
                append_move(src.id, angle, atk_left)
                planned_commitments[target.id].append((turns, world.player, int(atk_left)))
...
def _emergency_fallback(world):
    """
    Ultra-cheap fallback that returns valid moves when plan_moves crashes
    or runs catastrophically over budget. Sends ~half of each planet's ships
    toward the nearest reachable non-friendly planet.
    """
    moves = []
    if world is None:
        return moves
    ...
    for src in world.my_planets:
        ...
        for tgt in targets:
            ...
            if d >= best_d:
                continue
            est = estimate_arrival(src.x, src.y, src.radius, tgt.x, tgt.y, tgt.radius, send)
            if est is None:
                continue
            best_d = d
            best_angle = est[0]
        if best_angle is None:
            continue
        moves.append([src.id, float(best_angle), int(send)])
    return moves
...
def agent(obs, config=None):
    ...
    try:
        ...
        soft_budget = max(SOFT_ACT_BUDGET_MIN, float(act_timeout) * SOFT_ACT_BUDGET_RATIO)
        deadline = start_time + soft_budget
        result = plan_moves(world, deadline=deadline)
        return result if result is not None else []
    except Exception:
        # Last-resort recovery: if anything blew up, ship the simplest valid
        # action so we don't get INVALID-flagged on Kaggle.
        try:
            return _emergency_fallback(world)
        except Exception:
            return []
