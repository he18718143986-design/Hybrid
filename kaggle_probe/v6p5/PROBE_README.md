...
- **WEAKEST_ENEMY_VALUE_MULT_4P 1.5→1.7**
- 源文件：`submission_ablation_v6p5.py`
- 候选 SHA256（短）：`3c787c226687`
...
.venv/bin/kaggle competitions submit orbit-wars \
  -f kaggle_probe/v6p5/submission.py \
  -m "probe v6p5: WEAKEST_ENEMY_VALUE_MULT_4P 1.5→1.7 (base v2 @ 710d3ff438a2)"
```
...
2. 将 `kaggle_probe/v6p5/submission.py` 全文复制进一个 cell（保留首行 `# %%writefile submission.py` 若存在）  
3. Run → **Save Version** → **Submit to Competition**  
4. Description 填：`probe v6p5: WEAKEST_ENEMY_VALUE_MULT_4P 1.5→1.7 (base v2 @ 710d3ff438a2)`
...
- 每天最多 **5** 次提交；**一次只探一个 id**（先 v6p5，再 v6p5、v6p6）。
