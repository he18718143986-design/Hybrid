...
# v5-safe — v2 (814) + swarm_tol=2 + ENDGAME dispatcher + perf_tight + fallback
import math
import time
...
TOTAL_STEPS = 500
SIM_HORIZON = 110
ROUTE_SEARCH_HORIZON = 60           # v2 baseline — early/mid intercept search
ROUTE_SEARCH_HORIZON_LATE = 55      # late congested board only
HORIZON = SIM_HORIZON

# Late-game perf triggers — tighter than v4: require step + congestion
LATE_GAME_STEP_THRESHOLD = 280
CONGESTED_FLEET_COUNT = 10
...
# Hostile reinforcement — v2 proven config (timeline model only, no margin layer)
HOSTILE_REINFORCE_FRACTION = 0.5
HOSTILE_REINFORCE_MAX_SOURCES = 3
HOSTILE_REINFORCE_MIN_SHIPS = 4
HOSTILE_REINFORCE_SLACK = 0
...
SOFT_ACT_BUDGET_RATIO = 0.82       # v2-equivalent (actTimeout=1s → 0.82s budget)
SOFT_ACT_BUDGET_MIN = 0.55
HEAVY_PHASE_MIN_TIME = 0.16
HEAVY_PHASE_MIN_TIME_LATE = 0.20
OPTIONAL_PHASE_MIN_TIME = 0.08
OPTIONAL_PHASE_MIN_TIME_LATE = 0.10
HEAVY_ROUTE_PLANET_LIMIT = 28
HEAVY_ROUTE_PLANET_LIMIT_LATE = 20
...
def _set_perf_context(world):
    global _perf_tight
    congested = (
        len(world.fleets) >= CONGESTED_FLEET_COUNT
        or len(world.planets) >= CONGESTED_PLANET_COUNT
    )
    _perf_tight = world.step >= LATE_GAME_STEP_THRESHOLD and congested
...
def _allow_four_source():
    return FOUR_SOURCE_SWARM_ENABLED and not _is_perf_tight()
...
class WorldModel:
    ...
    def hostile_reinforcement_arrivals(self, target_id, attack_turn):
        ...
        result = tuple((eta, owner, ships) for eta, ships in candidates)
...
def plan_moves(world, deadline=None):
    ...
    # === TOTAL WAR ENDGAME ===
    # Value/cost ranked candidates; sizes attacks via min_ships_to_own_at.
...
def _emergency_fallback(world):
    """Ultra-cheap fallback when plan_moves crashes or over budget."""
    moves = []
    if world is None:
        return moves
