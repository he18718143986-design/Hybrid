...
# merged: 814-base + parameter tuning + new logic
...
ROTATION_LIMIT = 50.0
TOTAL_STEPS = 500
SIM_HORIZON = 130            # ↑ 110→130
ROUTE_SEARCH_HORIZON = 75    # ↑ 60→75
HORIZON = SIM_HORIZON
LAUNCH_CLEARANCE = 0.1

EARLY_TURN_LIMIT = 40
OPENING_TURN_LIMIT = 80
LATE_REMAINING_TURNS = 75            # ↑ 70→75
VERY_LATE_REMAINING_TURNS = 25
TOTAL_WAR_REMAINING_TURNS = 65       # ↑ 55→65

SAFE_NEUTRAL_MARGIN = 2
CONTESTED_NEUTRAL_MARGIN = 2
INTERCEPT_TOLERANCE = 1

SAFE_OPENING_PROD_THRESHOLD = 3      # ↓ 4→3
SAFE_OPENING_TURN_LIMIT = 14         # ↑ 10→14
ROTATING_OPENING_MAX_TURNS = 20      # ↑ 13→20
ROTATING_OPENING_LOW_PROD = 2
FOUR_PLAYER_ROTATING_REACTION_GAP = 2    # ↓ 3→2
FOUR_PLAYER_ROTATING_SEND_RATIO = 0.65   # ↑ 0.55→0.65
FOUR_PLAYER_ROTATING_TURN_LIMIT = 18     # ↑ 10→18

COMET_MAX_CHASE_TURNS = 10

ATTACK_COST_TURN_WEIGHT = 0.44       # ↓ 0.50→0.44
SNIPE_COST_TURN_WEIGHT = 0.37        # ↓ 0.42→0.37
INDIRECT_VALUE_SCALE = 0.15
INDIRECT_FRIENDLY_WEIGHT = 0.35
INDIRECT_NEUTRAL_WEIGHT = 0.9
INDIRECT_ENEMY_WEIGHT = 1.25

STATIC_NEUTRAL_VALUE_MULT = 1.6      # ↑ 1.4→1.6
STATIC_HOSTILE_VALUE_MULT = 1.85     # ↑ 1.65→1.85
ROTATING_OPENING_VALUE_MULT = 0.95   # ↑ 0.9→0.95
HOSTILE_TARGET_VALUE_MULT = 2.2      # ↑ 2.05→2.2
OPENING_HOSTILE_TARGET_VALUE_MULT = 1.65  # ↑ 1.55→1.65
SAFE_NEUTRAL_VALUE_MULT = 1.28       # ↑ 1.2→1.28
CONTESTED_NEUTRAL_VALUE_MULT = 0.78  # ↑ 0.7→0.78
EARLY_NEUTRAL_VALUE_MULT = 1.28      # ↑ 1.2→1.28
COMET_VALUE_MULT = 0.65
SNIPE_VALUE_MULT = 1.18              # ↑ 1.12→1.18
SWARM_VALUE_MULT = 1.10              # ↑ 1.05→1.10
REINFORCE_VALUE_MULT = 1.35
CRASH_EXPLOIT_VALUE_MULT = 1.28      # ↑ 1.18→1.28
FINISHING_HOSTILE_VALUE_MULT = 1.42  # ↑ 1.3→1.42
BEHIND_ROTATING_NEUTRAL_VALUE_MULT = 0.92
EXPOSED_PLANET_VALUE_MULT = 2.5      # ↑ 2.0→2.5

WEAKEST_ENEMY_VALUE_MULT_4P = 1.7    # ↑ 1.5→1.7
WEAKEST_ENEMY_VALUE_MULT_2P = 1.35   # ↑ 1.25→1.35
GANG_UP_VALUE_MULT = 1.75            # ↑ 1.4→1.75
GANG_UP_POST_BATTLE_DELAY = 1        # already 1 in 814
GANG_UP_ETA_WINDOW = 6               # ↑ 4→6

# NEW: competition pressure for neutrals the enemy reaches first
COMPETITION_PRESSURE_HARD = 0.28
COMPETITION_PRESSURE_SOFT = 0.62
# NEW: high-production planet compounding bonus
HIGH_PROD_BONUS_THRESHOLD = 3
HIGH_PROD_BONUS_PER_POINT = 0.12
# NEW: production-deficit urgency bonus
PROD_BEHIND_NEUTRAL_MULT = 1.18

NEUTRAL_MARGIN_BASE = 3              # ↑ 2→3
NEUTRAL_MARGIN_PROD_WEIGHT = 2
NEUTRAL_MARGIN_CAP = 8
HOSTILE_MARGIN_BASE = 4              # ↑ 3→4
HOSTILE_MARGIN_PROD_WEIGHT = 2
HOSTILE_MARGIN_CAP = 12
STATIC_TARGET_MARGIN = 4
CONTESTED_TARGET_MARGIN = 5
FOUR_PLAYER_TARGET_MARGIN = 3        # ↑ 2→3
...
FOUR_PLAYER_ROTATING_NEUTRAL_SCORE_MULT = 0.88   # ↑ 0.84→0.88
...
REAR_SEND_RATIO_TWO_PLAYER = 0.68    # ↑ 0.62→0.68
REAR_SEND_RATIO_FOUR_PLAYER = 0.65   # ↑ 0.60→0.65
...
PROACTIVE_DEFENSE_HORIZON = 16       # ↑ 12→16
PROACTIVE_DEFENSE_RATIO = 0.35       # ↑ 0.28→0.35
MULTI_ENEMY_PROACTIVE_HORIZON = 18   # ↑ 14→18
MULTI_ENEMY_PROACTIVE_RATIO = 0.45   # ↑ 0.35→0.45
MULTI_ENEMY_STACK_WINDOW = 5         # ↑ 4→5
REACTION_SOURCE_TOP_K_MY = 4
REACTION_SOURCE_TOP_K_ENEMY = 4
PROACTIVE_ENEMY_TOP_K = 3

CRASH_EXPLOIT_ENABLED = True
CRASH_EXPLOIT_MIN_TOTAL_SHIPS = 5    # ↓ 7→5
CRASH_EXPLOIT_ETA_WINDOW = 5         # ↑ 3→5
CRASH_EXPLOIT_POST_CRASH_DELAY = 1

# Hostile reinforcement prediction (from 814 version)
...
WEAK_ENEMY_THRESHOLD = 140           # ↑ 110→140
ELIMINATION_BONUS = 80.0             # ↑ 55.0→80.0
...
SOFT_ACT_DEADLINE = 0.88             # ↑ 0.82→0.88
HEAVY_PHASE_MIN_TIME = 0.16
OPTIONAL_PHASE_MIN_TIME = 0.08
HEAVY_ROUTE_PLANET_LIMIT = 40        # ↑ 32→40
...
def fleet_target_planet(fleet, planets, initial_by_id, angular_velocity, comets, comet_ids):
    ...
    Static planets: analytical ray-vs-circle (fast, exact).
    Rotating planets / comets: step through time and check predicted disc.
    """
...
def detect_enemy_planet_battles(world):
    """Uses projected_state for accurate post-battle estimation (from 814)."""
    battles = []
    seen = set()
...
def build_modes(world):
    ...
    # NEW: detect production deficit for urgency bonus
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    # NEW: high-production compounding bonus
    if target.production > HIGH_PROD_BONUS_THRESHOLD:
        extra = target.production - HIGH_PROD_BONUS_THRESHOLD
        value *= 1.0 + extra * HIGH_PROD_BONUS_PER_POINT
    ...
    if target.owner == -1:
        ...
        # NEW: competition pressure — enemy reaches neutral before our fleet
        if mission not in ("snipe",):
            _, enemy_t = policy_reaction_times(target.id, policy)
            if enemy_t < 10**9:
                if arrival_turns > enemy_t + 4:
                    value *= COMPETITION_PRESSURE_HARD   # 0.28
                elif arrival_turns > enemy_t + 1:
                    value *= COMPETITION_PRESSURE_SOFT   # 0.62

        # NEW: urgency bonus when behind on production
...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    ...
    # ↑ larger reductions for exposed / weakest targets
    if target.id in world.exposed_planet_ids:
...
def plan_moves(world, deadline=None):
                    ...
                    if time_filters_pass(target, p_turns, global_needed, src_available):
                        ...
                        if partial_value > 0:
                            ...
                            source_options_by_target[target.id].append(
                                ShotOption(
                                    score=partial_score,
                                    src_id=src.id, target_id=target.id,
                                    angle=p_angle, turns=p_turns,
                                    needed=global_needed, send_cap=partial_send_cap,
                                    mission="swarm",
                                )
                            )
        ...
        if mission.kind in ("single", "snipe", "rescue", "recapture", "reinforce", "crash_exploit"):
            ...
            if mission.kind == "reinforce":
                ...
                plan = settle_plan(
                    ...
                    world, planned_commitments, modes, policy,
                    mission="crash_exploit",
                    eval_turn_fn=lambda turns, da=option.anchor_turn: max(turns, da),
                    anchor_turn=option.anchor_turn,
                    anchor_tolerance=CRASH_EXPLOIT_ETA_WINDOW,
                )
            else:
    ...
    if world.is_total_war and world.enemy_planets and allow_optional_phase():
        if world._weakest_enemy is not None:
            ...
            primary_targets = weakest_planets if weakest_planets else world.enemy_planets
        else:
